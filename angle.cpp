...
static inline cv::Vec3f euclidToHomo(const cv::Vec4f & arg) {
    cv::Vec3f ret;
    ret[0] = arg[0] / arg[3];
    ret[1] = arg[1] / arg[3];
    ret[2] = arg[2] / arg[3];
...
int main(int argc, char* argv[])
{
    ...
    for (; 'q' != (char) cv::waitKey(10); ) {
        ...
        for (auto& kv : chilitags3D.estimate(inputImage)) {
            ...
            static const float DEFAULT_SIZE = 40.f;
            ...
            cv::Vec4f originVector = (transformation * UNITS[0]);
            cv::Vec4f pointVector = (transformation * UNITS[3]);

            // Transform homogenous coordinates to euclidean coordinates
            cv::Vec3f a = { 0.f, 0.f, 1.f };
            cv::Vec3f b = euclidToHomo(pointVector) - euclidToHomo(originVector);
            double angle = std::acos(a.dot(b) /
                    (cv::norm(a) * cv::norm(b))) * 180 / M_PI;

            // calculate the angle between a and b, report it
